...
const ZonesPage = () => {
  ...
  const { getAllZones, zones, searchZones, loading } = useDashboardHook();
      ...
      <Modal open={open} onCancel={() => setOpen(false)} footer={null}>
        <h1 className='text-2xl text-slate-400'>Update Post </h1>
        {
          ...
          <form action="" className='mt-10' onSubmit={async (e: any) => {
            ...
            <input type="submit" value={loading ? "please wait..." : "Update Post"} disabled={loading} className='bg-yellow-400 py-1 px-4 rounded-md my-3 cursor-pointer' />
          </form>
        }
      </Modal>
    </DashboardLayout>
